...
extern IMAGE img_2P_selector_btn_down_right; // 2P 向右选择按钮按下状态图
...
extern Atlas atlas_peashooter_idle_right; // 婉逗射手朝向左的默认动画图集
extern Atlas atlas_sunflower_idle_right; // 龙日葵朝向左的默认动画图集

extern IMAGE img_avatar_peashooter; // 婉逗射手头像图片
extern IMAGE img_avatar_sunflower; // 龙日葵头像图片

extern SceneManger scene_manager;
...
public:
	...
	void on_draw(const Camera& camera)
	{
		...
		case PlayerType::Peashooter:
			...
			pos_img_1P_name.x = pos_img_1P_gravestone.x + (img_gravestone_right.getwidth() - textwidth(str_pesshooter_name)) / 2;
			outtextxy_shaded(pos_img_1P_name.x, pos_img_1P_name.y, str_pesshooter_name);
			break;
		case PlayerType::Sunflower:
		...
		case PlayerType::Peashooter:
			...
			pos_img_2P_name.x = pos_img_2P_gravestone.x + (img_gravestone_right.getwidth() - textwidth(str_pesshooter_name)) / 2;
			outtextxy_shaded(pos_img_2P_name.x, pos_img_2P_name.y, str_pesshooter_name);
			break;
		case PlayerType::Sunflower:
	...
	void on_input(const ExMessage& msg)
	{
		...
		case WM_KEYUP:
			...
			case VK_RIGHT:
				...
				break;
			case VK_RETURN:
				scene_manager.switch_to(SceneManger::SceneType::Game);
...
private:
	...
	LPCTSTR str_pesshooter_name = _T("婉逗射手");
