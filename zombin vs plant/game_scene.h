...
public:
	...
	void on_enter()
	{
		...
		timer_winner_slide_out.set_callback([&]() {
			scene_manager.switch_to(SceneManger::SceneType::Menu);
			});
	...
	void on_update(int delta)
	{
		...
		bullet_list.erase(std::remove_if(bullet_list.begin(), bullet_list.end(),
			[](const Bullet* bullet)
			{
				bool deletable = bullet->check_can_move();
...
private:
	...
	StatuBar status_bar_1p;
	StatuBar status_bar_2p;

	bool is_geme_over = false;
